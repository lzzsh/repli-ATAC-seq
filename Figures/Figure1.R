library(dplyr)
gff<-read.table("~/Desktop/毕设文献/segamentation/rbq_gff.gff3",skip = 2,comment.char = "")
gff<-gff[,c(1,4,5,9)]
...
p5 <- ggpiestats(gff, 'RT',  
                 ...
                 title = 'Partition of total genome replication into seven RT segment classes'#设置标题
...
pic <- ggplot(data = gff, aes(x = RT, y = length))+   #指定数据集，设置坐标轴名称、类别颜色
  geom_boxplot(aes(color = RT))+ #绘制箱线图
