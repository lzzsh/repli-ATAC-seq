awk '{print $1"\t"$2"\t"$3}' peaks_quan_tpm_filtered_org.txt > peaks_quan_tpm_filtered_pos_org.txt
...
-bed peaks_quan_tpm_filtered_pos_org.txt  > peaks_re_quan_org.txt
...
bedtools multicov -bams \
	...
	/storage/liuxiaodongLab/liaozizhuo/Projects/repli-ATAC-seq_control/bwa_all_rawdata/LZZ-9-2_S99_sort_rmdup_rmor_q30.bam \
	/storage/liuxiaodongLab/liaozizhuo/Projects/repli-ATAC-seq_control/bwa_all_rawdata/LZZ-9-3_S99_sort_rmdup_rmor_q30.bam \
-bed peaks_quan_tpm_filtered_pos_org.txt  > peaks_re_quan_CR_org.txt
...
for file in ${files[@]}; do
	name=$(basename ${file})
	name=${name%%.*}
	bedtools intersect -abam ${file} -b peaks_quan_tpm_filtered_pos_org.txt -wa > ${name}_filtered_org.bam
    samtools index ${name}_filtered_org.bam
done

#mkdir bws
for file in $(find ./ -name '*_filtered_org.bam');do
	name=$(basename ${file})
	prefix=${name%_*}
	bamCoverage --bam ${file} -o ./bws/${prefix}_org.bw --normalizeUsing BPM -p 5 --binSize 50
	sleep 1
done