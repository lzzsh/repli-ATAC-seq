...
def test_transformer_block_single_ffn_dropout():
    """FFN path must have exactly one dropout (drop2), not two."""
    from src.models.model import _TransformerBlock
    block = _TransformerBlock(d_model=64, n_heads=4, dim_key=16)
    dropouts = [m for m in block.modules() if isinstance(m, torch.nn.Dropout)]
    # block owns: drop1, drop2 (2 direct)
    # _EnformerAttention owns: pos_dropout, attn_dropout (2 nested)
    # total = 4; previously 5 when drop3 existed
    assert len(dropouts) == 4, f"Expected 4 Dropout modules, got {len(dropouts)}"
