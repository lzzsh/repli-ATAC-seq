...
# 设置工作目录
setwd("/storage2/liuxiaodongLab/liaozizhuo/Projects/ATAC-seq-CR/signal/")
...
colnames(count_matrix) <- c(
  ...
  "TCX2_1-1-ES", "TCX2_1-2-ES", "TCX2_1-1-MS", "TCX2_1-1-LS", 
  "TCX2_3-1-all", "sol1_5-1-all"
)

# TPM 标准化函数
normalize_tpm <- function(data) {
  data$RegionLength <- data$end - data$start + 1
  data[, 4:13] <- lapply(data[, 4:13], function(col) {
