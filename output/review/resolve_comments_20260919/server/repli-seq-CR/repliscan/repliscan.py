...
Methods to handle replicates:
  ...
  - max''')
	...
	print("Skipping Haar Wavelet smoothing step.")
	gc.collect()
	#####################################################
	# Perform Segmentation
	#####################################################
	makeGFF_no_smooth(fList, chromDict, args.window, args.plot, args.threshold, args.scope, args.log, args.value, args.prep, args.classifier)
	print "Done"
...
def makeBedgraph(fList, fasta, size, aggMethod, normMethod, removeWhat, plotCoverage, pLow, pHigh):
	...
	for bams, prefix in fList:
	    print("[INFO] Aggregating group: %s" % prefix)
	    ret.append(agregate((bams, prefix, faiBed, aggMethod)))
...
def agregate(argList):
	...
	if not os.path.exists(finalBG) or force:
		if len(bgs) == 1:
			print("[INFO] Only one replicate, copying %s -> %s" % (bgs[0], finalBG))
			if os.path.exists(finalBG):
				os.remove(finalBG)
			import shutil
			shutil.copyfile(bgs[0], finalBG)
		else:
...
# Added: makeGFF_no_smooth for skipping smoothing step.
def makeGFF_no_smooth(fList, chromDict, S, plotCov, threshMethod, scope, useLog, thresh, pCut, segMeth):
    fSuff = {True:'logFC', False:'ratio'}[useLog]
    beds = map(lambda y: "%s_%s.bedgraph" % (y[1], fSuff), fList[1:])
    names = map(lambda y: y[1], fList[1:])
    plotColors = plotVars(names)

    for name in names:
        outGFF = '%s_%s.gff3' % (name, fSuff)
        open(outGFF,'w').write('##gff-version 3\n#track name="%s %ibp" gffTags=on\n' % (name,S))
    ...
    for chrom in sortedChroms:
        ...
        for i in xrange(len(beds)):
            colorIndex = int(''.join([ '1' if i == j else '0' for j in range(len(beds))]),2)
            outGFF = '%s_%s.gff3' % (names[i], fSuff)
            outSignal = vals[i,s:e]
            bMask = np.zeros(len(outSignal), dtype=np.bool)
            bMask[outSignal > thresh] = 1
            maskM[i,:] = bMask[:]
