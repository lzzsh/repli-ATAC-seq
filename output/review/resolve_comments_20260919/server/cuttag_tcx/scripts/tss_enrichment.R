...
setwd("/storage2/liuxiaodongLab/liaozizhuo/Projects/cuttag_tcx/tss_plot")
gff3 <- read.table('/storage2/liuxiaodongLab/liaozizhuo/Projects/repli-ATAC-seq/reference/all_DIY.gff3',stringsAsFactors=F,sep='\t')
ids <- gff3$V3 == 'gene'
...
pdf(file = paste0(data_name, '_tss.pdf'))
...
#file_list(Args[6])
files <- list.files('/storage/liuxiaodongLab/liaozizhuo/Projects/repli-seq-pre-2/bwa_all_rawdata','*_q30.bam',full.names=T)
file_list(files)
